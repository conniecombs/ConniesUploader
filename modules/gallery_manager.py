...
class GalleryManager(ctk.CTkToplevel):
    ...
    def _init_ui(self):
        ...
        ctk.CTkButton(bottom, text="Create Gallery", command=self._create_gallery, fg_color="green").pack(fill="x", padx=5, pady=5)
    ...
    def _render_list(self, data, append=False):
        ...
        if not data:
            if not append:
                ...
                if self.service_var.get() == "imx.to":
                    btn = ctk.CTkButton(self.scroll, text="Login Failed? Set Cookies Manually", 
                                      command=self._ask_cookies_dialog, fg_color="orange", text_color="black")
                    btn.pack(pady=20)
        ...
        for item in data:
            ...
            if self.callback:
                ctk.CTkButton(f, text="Select", width=60, height=24, 
                            command=lambda s=self.service_var.get(), g=gid: self._select(s, g)).pack(side="right", padx=5)

        # Append "Load More" button at the bottom if we found data
        # (Assuming if we found data, there *might* be another page)
        if self.service_var.get() == "imx.to":
            self.btn_load_more = ctk.CTkButton(self.scroll, text="Load Next Page", 
                                             command=self._load_more_pages, fg_color="#3B8ED0")
            self.btn_load_more.pack(pady=15)

    def _select(self, service, gid):
        if self.callback: self.callback(service, gid)
        self.destroy()
    ...
    def _fetch_galleries(self, service, page=1):
        if service == "imx.to": return self._fetch_imx_galleries(page)
        ...
        if resp.get("type") == "data" and resp.get("data"): return resp["data"]
        return []

    def _perform_create(self, service, name):
        if service == "imx.to": return self._create_imx_gallery(name)
        payload = { "action": "create_gallery", "service": service, "creds": self._get_creds_dict(), "config": {"gallery_name": name} }
        resp = self.bridge.request_sync(payload, timeout=20)
        if resp.get("status") == "success": return resp.get("data")
        if service == "pixhost.to":
            try:
                client = api.create_resilient_client()
                res = api.create_pixhost_gallery(name, client)
                client.close()
                if res: return res.get('gallery_hash')
            except: pass
        return None
    ...
    def _find_gid_in_list(self, session, name):
        try:
            ...
            for gid, gname in re.findall(pattern, r.text, re.IGNORECASE):
                if gname.strip() == name: return gid
        except: pass
        return None