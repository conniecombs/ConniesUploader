...
class NativeComboBox(ttk.Combobox):
    def __init__(self, master=None, **kwargs):
        ...
        if "variable" in kwargs: kwargs["textvariable"] = kwargs.pop("variable")
        
        ctk_args = ['text_color', 'fg_color', 'border_color', 'button_color', 
                    'button_hover_color', 'dropdown_fg_color', 'dropdown_hover_color', 
                    'dropdown_text_color', 'dropdown_text_font', 'font', 
...
class MouseWheelComboBox(ctk.CTkComboBox):
    ...
    def _on_mouse_wheel(self, event, linux_delta=None):
        ...
        try: current_val = self.get(); current_idx = values.index(current_val)
...
class ServiceSettingsView:
    ...
    def _init_variables(self):
        # IMX
        if not hasattr(self.app, 'var_imx_thumb'): self.app.var_imx_thumb = ctk.StringVar(value="180")
        if not hasattr(self.app, 'var_imx_format'): self.app.var_imx_format = ctk.StringVar(value="Fixed Width")
        if not hasattr(self.app, 'var_imx_cover_count'): self.app.var_imx_cover_count = ctk.StringVar(value="0") 
        if not hasattr(self.app, 'var_imx_links'): self.app.var_imx_links = ctk.BooleanVar()
        if not hasattr(self.app, 'var_imx_threads'): self.app.var_imx_threads = ctk.IntVar(value=5)

        # Pixhost
        if not hasattr(self.app, 'var_pix_content'): self.app.var_pix_content = ctk.StringVar(value="Safe")
        if not hasattr(self.app, 'var_pix_thumb'): self.app.var_pix_thumb = ctk.StringVar(value="200")
        if not hasattr(self.app, 'var_pix_cover_count'): self.app.var_pix_cover_count = ctk.StringVar(value="0") 
        if not hasattr(self.app, 'var_pix_links'): self.app.var_pix_links = ctk.BooleanVar()
        if not hasattr(self.app, 'var_pix_threads'): self.app.var_pix_threads = ctk.IntVar(value=3)
        
        # Turbo
        if not hasattr(self.app, 'var_turbo_content'): self.app.var_turbo_content = ctk.StringVar(value="Safe")
        if not hasattr(self.app, 'var_turbo_thumb'): self.app.var_turbo_thumb = ctk.StringVar(value="180")
        if not hasattr(self.app, 'var_turbo_cover_count'): self.app.var_turbo_cover_count = ctk.StringVar(value="0") 
        if not hasattr(self.app, 'var_turbo_links'): self.app.var_turbo_links = ctk.BooleanVar()
        if not hasattr(self.app, 'var_turbo_threads'): self.app.var_turbo_threads = ctk.IntVar(value=2)
        
        # Vipr
        if not hasattr(self.app, 'var_vipr_thumb'): self.app.var_vipr_thumb = ctk.StringVar(value="170x170")
        if not hasattr(self.app, 'var_vipr_gallery'): self.app.var_vipr_gallery = ctk.StringVar()
        if not hasattr(self.app, 'var_vipr_cover_count'): self.app.var_vipr_cover_count = ctk.StringVar(value="0") 
        if not hasattr(self.app, 'var_vipr_links'): self.app.var_vipr_links = ctk.BooleanVar()
        if not hasattr(self.app, 'var_vipr_threads'): self.app.var_vipr_threads = ctk.IntVar(value=1)
        
        # ImageBam
        if not hasattr(self.app, 'var_ib_content'): self.app.var_ib_content = ctk.StringVar(value="Safe")
        if not hasattr(self.app, 'var_ib_thumb'): self.app.var_ib_thumb = ctk.StringVar(value="180")
        if not hasattr(self.app, 'var_ib_threads'): self.app.var_ib_threads = ctk.IntVar(value=2)
    ...
    def _build_imx(self):
        ...
        MouseWheelComboBox(p, variable=self.app.var_imx_thumb, values=["100","180","250","300","600"]).pack(fill="x")
        ctk.CTkLabel(p, text="Format:").pack(anchor="w")
        MouseWheelComboBox(p, variable=self.app.var_imx_format, values=["Fixed Width", "Fixed Height", "Proportional", "Square"]).pack(fill="x")
    ...
    def _build_pix(self):
        ...
        ctk.CTkLabel(p, text="Thumb Size:").pack(anchor="w")
        MouseWheelComboBox(p, variable=self.app.var_pix_thumb, values=["150","200","250","300","350","400","450","500"]).pack(fill="x")
    ...
    def _build_turbo(self):
        ...
        ctk.CTkLabel(p, text="Thumb Size:").pack(anchor="w")
        MouseWheelComboBox(p, variable=self.app.var_turbo_thumb, values=["150","200","250","300","350","400","500","600"]).pack(fill="x")
    ...
    def _build_vipr(self):
        ...
        ctk.CTkLabel(p, text="Requires Credentials", text_color="red").pack(pady=5)
        ctk.CTkLabel(p, text="Thumb Size:").pack(anchor="w")
        MouseWheelComboBox(p, variable=self.app.var_vipr_thumb, values=["100x100", "170x170", "250x250", "300x300", "350x350", "500x500", "800x800"]).pack(fill="x")
        ...
        ctk.CTkButton(p, text="Refresh Galleries / Login", command=self.app.refresh_vipr_galleries).pack(fill="x", pady=10)
...
class CollapsibleGroupFrame(ctk.CTkFrame):
    def __init__(self, parent, title="Group", thread_names=None, template_names=None, default_template="BBCode"):
        ...
        vals = ["Do Not Post"]
        if thread_names: vals += sorted(thread_names)
        self.thread_var = ctk.StringVar(value="Do Not Post")
        self.thread_combo = NativeComboBox(self.header, width=20, values=vals, variable=self.thread_var, command=self._on_thread_change)
        self.thread_combo.pack(side="right", padx=5)

        if template_names:
            val = default_template if default_template in template_names else (template_names[0] if template_names else "")
            self.selected_template = val
            self.template_var = ctk.StringVar(value=val)
            self.template_combo = NativeComboBox(self.header, width=15, values=template_names, variable=self.template_var, command=self._on_template_change)
...
class LogWindow(ctk.CTkToplevel):
    def __init__(self, parent, initial_logs=[]):
        ...
        for line in initial_logs: self.append_log(line)
