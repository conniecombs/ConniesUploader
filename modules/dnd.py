...
class DragDropMixin:
    ...
    def _on_group_drag_motion(self, event):
        if self.drag_data["type"] != "group": return
        y_root = event.y_root
        
        for target in self.groups:
            if target == self.drag_data["item"]: continue
            ...
            if y_root > t_y and y_root < t_y + t_h:
                ...
                for g in self.groups: g.pack_forget()
                for g in self.groups: g.pack(fill="x", pady=2, padx=2)
                break
    ...
    def _find_target_row_and_group(self, widget):
        ...
        if isinstance(widget, str):
            try: widget = self.nametowidget(widget)
    ...
    def _move_file_to_group(self, fp, old_group, new_group, before_widget=None):
        ...
        if before_widget:
            try: new_row.pack(before=before_widget)
            except: pass
