...
class TemplateManager:
    ...
    def process_conditionals(self, template_content: str, data: Dict[str, Any]) -> str:
        ...
        while iteration < max_iterations:
            ...
            else_match = re.match(r"^(.*?)\[else\](.*?)$", conditional_block, re.DOTALL)
...
class TemplateEditor(ctk.CTkToplevel):
    ...
    def _init_ui(self):
        ...
        for text, value in vars_to_add:
            ctk.CTkButton(var_bar, text=text, width=70, height=24, command=lambda v=value: self.ins(v)).pack(side="left", padx=2)
