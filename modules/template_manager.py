...
class TemplateEditor(ctk.CTkToplevel):
    ...
    def get_tags(self, mode, value=None):
        fmt = self.fmt.get()
        if mode == "Bold": return ("[b]", "[/b]") if fmt == "BBCode" else ("**", "**") if fmt == "Markdown" else ("<b>", "</b>")
        elif mode == "Italic": return ("[i]", "[/i]") if fmt == "BBCode" else ("*", "*") if fmt == "Markdown" else ("<i>", "</i>")
        elif mode == "Underline": return ("[u]", "[/u]") if fmt == "BBCode" else ("<u>", "</u>")
        elif mode == "Color": return (f"[color={value}]", "[/color]") if fmt == "BBCode" else (f'<span style="color:{value}">', "</span>")
        elif mode == "Size": return (f"[size={value}]", "[/size]") if fmt == "BBCode" else (f'<span style="font-size:{value}px">', "</span>")
        elif mode == "Font": return (f"[font={value}]", "[/font]") if fmt == "BBCode" else (f'<span style="font-family:{value}">', "</span>")
        return ("", "")

    def format_text(self, mode):
        try:
            start = self.txt.index("sel.first"); end = self.txt.index("sel.last")
            s_tag, e_tag = self.get_tags(mode)
            selected = self.txt.get(start, end)
            self.txt.delete(start, end); self.txt.insert(start, f"{s_tag}{selected}{e_tag}")
        except tk.TclError: self.txt.insert("insert", "".join(self.get_tags(mode)))

    def format_complex(self, mode):
        val = None
        if mode == "Color":
            c = colorchooser.askcolor(title="Select Color")
            if c and c[1]: val = c[1]
        if val: self.apply_from_combo(mode, val)
    ...
    def apply_from_combo(self, mode, value):
        ...
        try:
            ...
            selected = self.txt.get(start, end)
            self.txt.delete(start, end); self.txt.insert(start, f"{s_tag}{selected}{e_tag}")
        except tk.TclError: self.txt.insert("insert", "".join(self.get_tags(mode, value)))

    def ins(self, v): self.txt.insert("insert", v); self.txt.focus()
    def load_curr(self, _=None): self.txt.delete("0.0", "end"); self.txt.insert("0.0", self.mgr.get_template(self.fmt.get()))
    def load_saved_template(self):
        sel = self.saved_tmpl_var.get()
        if not sel: return
        self.txt.delete("0.0", "end"); self.txt.insert("0.0", self.mgr.get_template(sel))
        if sel not in self.cb_fmt._values: self.cb_fmt.configure(values=self.cb_fmt._values + [sel])
        self.cb_fmt.set(sel); self.fmt.set(sel)
    def save(self):
        name = self.fmt.get()
        self.mgr.set_template(name, self.txt.get("0.0", "end").strip())
        messagebox.showinfo("Saved", f"Template '{name}' updated.")
        if self.update_callback: self.update_callback(name)
    def save_as_new(self):
        ...
        if new_name:
            ...
            self.cb_saved.configure(values=keys); self.cb_fmt.configure(values=keys)
            self.cb_fmt.set(new_name); self.cb_saved.set(new_name); self.fmt.set(new_name)
            messagebox.showinfo("Success", f"Created: {new_name}")
            if self.update_callback: self.update_callback(new_name)
    def generate_preview(self):
        if not self.data_callback: return
        files, title, size = self.data_callback()
        if not files: return messagebox.showwarning("Preview", "Add files first.")
        if not size or not str(size).isdigit(): size = "200"
        ...
        ctx = {"gallery_link": "http://localhost/preview", "gallery_name": title, "gallery_id": "PREV_123", "cover_url": mock[0][1] if mock else ""}
        try: raw = self.mgr.apply(curr_fmt, ctx, mock)
        finally: self.mgr.set_template(curr_fmt, orig)
        ...
        try:
            with tempfile.NamedTemporaryFile('w', delete=False, suffix='.html', encoding='utf-8') as f:
                f.write(final_html); path = f.name
