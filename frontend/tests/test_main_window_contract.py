...
def test_completion_summary_renders_inline_with_relevant_actions(tmp_path):
    ...
    app.btn_completion_schedule = FakeFrame()
    summary = {
        "uploaded_count": 3,
        "failed_count": 1,
        "generated_count": 1,
    ...
    assert app.completion_panel.pack_kwargs["before"] is app.queue_footer
    ...
    assert app.btn_completion_schedule.mapped is True
    ...
    assert app.btn_completion_schedule.mapped is False
...
@pytest.mark.unit
def test_turbo_result_gallery_url_updates_group_metadata():
    gallery_url = "https://www.turboimagehost.com/album/394761/miscellaneous"
    group = FakeGroup("Turbo Batch", ["image.jpg"])
    app = UploaderApp.__new__(UploaderApp)
    app.progress_queue = queue.Queue()
    app.settings = {"service": "turboimagehost"}
    app.file_widgets = {"image.jpg": {"group": group}}
    activity = []
    app.add_activity = lambda message, level="info": activity.append((message, level))

    app.progress_queue.put(("gallery_url", "image.jpg", gallery_url))

    UploaderApp._process_progress_queue(app)

    assert group.gallery_url == gallery_url
    assert group.gallery_id == "394761"
    assert group.gallery_name == "Turbo Batch"
    assert group.gallery_service == "turboimagehost"
    assert activity == [("Turbo gallery URL captured for Turbo Batch.", "info")]
