...
func TestWorkerPoolConcurrency(t *testing.T) {
	...
	// Run upload (testing the worker pool pattern directly)
	start := time.Now()
...
func TestTimeoutBehavior(t *testing.T) {
	...
	// Create context with 120-second timeout
...
// TestProcessFileGenericWithTimeout tests the full processFileGeneric timeout mechanism
func TestProcessFileGenericWithTimeout(t *testing.T) {
	tmpDir := t.TempDir()
	...
	// Use a local server that immediately returns an error status
	...
	job := &JobRequest{
		Action:  "http_upload",
		Service: "unsupported_to_force_immediate_failure",
		...
		HttpSpec: &HttpRequestSpec{
			URL:    errorServer.URL + "/upload",
			Method: "POST",
			MultipartFields: map[string]MultipartField{
				"file": {Type: "file", Value: testFile},
			},
	...
	start := time.Now()
	processFileGeneric(testFile, job)
	duration := time.Since(start)

	// Should complete quickly with no retries against a local server
	if duration > 5*time.Second {
		t.Errorf("processFileGeneric took too long: %v (expected <5s with zero retries)", duration)
	}

	t.Logf("✓ processFileGeneric completed in: %v", duration)
}
